...
namespace FlowMy.Views
{
    ...
    public partial class WorkflowEditorWindow : Window
    {
        ...
        private bool ShouldRenderNodeInHeadlessMode(WorkflowNode node)
        {
            ...
            return node.Type == NodeType.HtmlUi || node.Type == NodeType.Web || node.Type == NodeType.ActionCanVas || node.Type == NodeType.ShowInputMsg || node.Type == NodeType.VideoEditor;
        }
            ...
            NodeDialogManager nodeDialogManager)
        {
            ...
            // Triple ESC within 2s: fires EscapeTriplePressed → stops workflow
            ...
            {
                // Install hook immediately so ESC triple-press works even when no key-wait node is running
            ...
            Closed += (_, _) =>
            {
                ...
                try { FlowMy.Views.Overlays.LayerAiWebViewCache.DisposeAll(); } catch { }
            };
        }
        ...
        private ImageProcessingNodeContentControl? GetImageEditorNodeUnderMouse()
        {
            ...
            if (hitTestResult?.VisualHit != null)
            {
                ...
                while (obj != null)
                {
                    if (obj is ImageProcessingNodeContentControl imageEditor)
                    {
                        return imageEditor;
                    }
