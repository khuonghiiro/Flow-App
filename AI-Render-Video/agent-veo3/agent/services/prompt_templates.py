...
ANGLE_PROMPT_TEMPLATES = {
    ...
    "135": (
        "FULL-BODY 135-DEGREE REAR THREE-QUARTER VIEW — 2D Xianxia anime chibi sprite.\n"
        "SINGLE REFERENCE: Use ONLY the 180° rear-view reference image.\n"
        "MANDATORY FULL-BODY 135-DEGREE ROTATION: The character's ENTIRE BODY (head, shoulders, torso, hips, legs, and feet) MUST be rotated to a 135-degree rear three-quarter angle. "
        "The character faces diagonally away towards 10 o'clock. "
        "STRICTLY FORBIDDEN to only rotate the head while keeping the body in a flat 180-degree rear pose! The torso, hips, and legs MUST ALL turn together. "
        "The left side of the back and left arm are in the foreground; the right shoulder and right arm recede deeply in perspective. "
        "Both feet point diagonally away towards 10 o'clock in stepped depth — left foot forward, right foot behind. "
        "STRICT GARMENT & COLOR LOCK: Must preserve 100% of the clothing details, colors ({primaryColor}), patterns, and garment layers from the 180° reference image. "
        "STRICTLY FORBIDDEN to alter robe colors or patterns. "
        "REAR WAIST: The waist sash and rear waist ({waistRearMotionLock}) are viewed at a 45-DEGREE ANGLED PERSPECTIVE (offset to the side, NOT centered flat). "
        "CRITICAL SCALE & STATURE LOCK: Maintain the EXACT SAME TALL, SLENDER, ATHLETIC HEIGHT AND PROPORTIONS (~88% vertical canvas height, ~4.8-5.0 heads tall). "
        "NATURAL FABRIC DRAPE: Hems hang straight down naturally under calm gravity. Strictly NO wind blowing, NO billowing fabric. "
        "CRITICAL — BLANK FACELESS HEAD: Smooth blank featureless face. Skin color matches neck ({skinTone}). "
        "CRITICAL — ZERO WEAPONS OR PROPS: NO weapons, NO props. Both hands empty, naturally posed. "
        "LIGHTING: Natural clean flat cel-shading, strictly ZERO neon lighting. "
        "IDENTITY LOCK: Match reference character (hairstyle {hairStyleColor}, outfit {outfitDescription}, colors {primaryColor}, accents {accentColor}). "
        "STYLE: Pure flat 2D anime chibi sprite illustration, clean linework, flat cel-shaded coloring. "
        "BACKGROUND: Solid chroma-key green {chromaBgHex}. Centered, full body visible."
...
IDLE_PROMPT_TEMPLATES = {
    ...
    "135": (
        ...
        "{waistRearMotionLock} Hair ornaments sway subtly. "
        "Static camera, seamless loop: first frame = last frame. Camera static. Solid green {chromaBgHex} background."
    ),
    "180": (
        ...
        "Hair ornaments sway subtly with the soft breeze. {waistRearMotionLock} "
        "Static camera, seamless loop: first frame = last frame. Camera static. Solid green {chromaBgHex} background."
    ),
}
