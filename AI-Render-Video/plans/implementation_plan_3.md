...
## 2. Hệ Thống Combat & Tác Động Lực Đồng Bộ (Combat Choreography)
...
### A. Sơ đồ luồng diễn biến đòn đánh (Visual Timeline Flow):

```
[ t = 5.0s : START ] ──────────────────────────────────────────► [ t = 5.4s : IMPACT POINT ]
  ├─ Nhân vật A: Bắt đầu vung kiếm 'heavy_slash'                   ├─ Khung hình chạm đích chính xác!
  └─ Gắn VFX: Vệt lửa 'sword_slash_fire' bám theo mũi kiếm         ├─ Nhân vật B: Bị đánh văng 'fly_back_knockdown'
                                                                   ├─ Dịch chuyển lùi: 'knockback_distance: 2.5m'
                                                                   ├─ Biểu cảm mặt: Đổi sang nhăn nhó 'pain'
                                                                   ├─ Spawn VFX: Tia lửa 'impact_hit_sparks' tại điểm chạm
                                                                   ├─ Camera: Rung lắc 'screen_shake' (intensity 0.35)
                                                                   └─ Audio: Phát tiếng kim loại va đập 'metal_hit.mp3'
```

### B. Sơ đồ Sequence Diagram:
...
### C. Diễn giải chi tiết từng thông số Combat trong JSON:
1. **`start_time` (t = 5.0s):** Thời điểm nhân vật A bắt đầu lấy đà vung kiếm -> Hệ thống tự động gắn VFX vệt sáng (`sword_slash_fire`) vào đầu mũi kiếm (`weapon_tip`).
2. **`impact_time` (t = 5.4s - Khung hình va chạm trúng đích):**
   * **Animation phản ứng:** Nhân vật B lập tức chuyển sang animation bị đánh văng (`fly_back_knockdown`) hoặc choáng váng (`stagger_back`).
   * **Tác động lực lùi (`knockback_distance: 2.5m`):** Nhân vật B bị đẩy lùi theo vector hướng tấn công.
   * **Cơ mặt biểu cảm (`facial_expression: "pain"`):** Chuyển ngay sang nhăn mặt đau đớn trong 0.8 giây.
   * **VFX Va chạm (`impact_hit_sparks`):** Bùng nổ tia lửa ngay tại điểm tiếp xúc giữa vũ khí và cơ thể đối thủ.
   * **Rung màn hình (`screen_shake`):** Rung ống kính máy quay (`intensity: 0.35, duration: 0.25s`) tạo cảm giác lực đánh cực mạnh.
   * **Âm thanh va đập:** Tự động phát sound effect va chạm kim loại/nắm đấm.

---

## 3. Hệ Thống Phụ Đề & Kiểm Tra Biểu Cảm (Subtitles & Lip-Sync Inspection)
