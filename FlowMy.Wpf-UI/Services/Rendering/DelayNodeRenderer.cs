...
{
    public sealed class DelayNodeRenderer : INodeRenderer
    {
        ...
        public DelayNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        {
            if (node is not DelayNode delayNode)
                return;

            var window = Host as Window;
            node.Border = DelayNodeControl.CreateBorder(delayNode, window, Host);
            ...
            if (delayNode.TitleTextBlockUI != null && !canvas.Children.Contains(delayNode.TitleTextBlockUI))
            {
                canvas.Children.Add(delayNode.TitleTextBlockUI);
                if (node.Border != null)
                {
                    var titleLeft = node.X + (node.Border.ActualWidth / 2) - (delayNode.TitleTextBlockUI.ActualWidth / 2);
                    var titleTop = node.Y - delayNode.TitleTextBlockUI.ActualHeight - 4;
                    Canvas.SetLeft(delayNode.TitleTextBlockUI, titleLeft);
                    Canvas.SetTop(delayNode.TitleTextBlockUI, titleTop);
                    Panel.SetZIndex(delayNode.TitleTextBlockUI, 20000);
                }
            }
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is DelayNode delayNode && delayNode.TitleTextBlockUI != null && Host.WorkflowCanvas != null)
            {
                var titleTextBlock = delayNode.TitleTextBlockUI;
        ...
        {
            if (node is DelayNode delayNode && delayNode.TitleTextBlockUI != null)
            {
                var titleTextBlock = delayNode.TitleTextBlockUI;
                if (canvas != null && canvas.Children.Contains(titleTextBlock))
                    canvas.Children.Remove(titleTextBlock);
                delayNode.TitleTextBlockUI = null;
            }
