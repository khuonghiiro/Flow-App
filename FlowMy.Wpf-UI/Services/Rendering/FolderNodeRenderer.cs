...
{
    public sealed class FolderNodeRenderer : INodeRenderer
    {
        ...
        public FolderNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        public void RenderNode(WorkflowNode node, Canvas canvas)
        {
            ...
            node.Border = FolderNodeControl.CreateBorder(folderNode, window, Host);
            ...
            if (folderNode.TitleTextBlockUI != null && !canvas.Children.Contains(folderNode.TitleTextBlockUI))
            {
                canvas.Children.Add(folderNode.TitleTextBlockUI);
                if (node.Border != null)
                {
                    var titleLeft = node.X + (node.Border.ActualWidth / 2) - (folderNode.TitleTextBlockUI.ActualWidth / 2);
                    var titleTop = node.Y - folderNode.TitleTextBlockUI.ActualHeight - 4;
                    Canvas.SetLeft(folderNode.TitleTextBlockUI, titleLeft);
                    Canvas.SetTop(folderNode.TitleTextBlockUI, titleTop);
                    Panel.SetZIndex(folderNode.TitleTextBlockUI, 20000);
                }
            }
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is FolderNode folderNode && folderNode.TitleTextBlockUI != null && Host.WorkflowCanvas != null)
            {
        ...
        {
            if (node is FolderNode folderNode && folderNode.TitleTextBlockUI != null)
            {
                var titleTextBlock = folderNode.TitleTextBlockUI;
