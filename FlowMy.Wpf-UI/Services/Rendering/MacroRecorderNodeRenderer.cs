...
{
    public sealed class MacroRecorderNodeRenderer : INodeRenderer
    {
        ...
        public MacroRecorderNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        {
            if (node is not MacroRecorderNode macroNode) return;

            // Tạo UI
            node.Border = MacroRecorderNodeControl.CreateBorder(macroNode, Application.Current?.MainWindow, Host);
            ...
            if (macroNode.TitleTextBlockUI != null && !canvas.Children.Contains(macroNode.TitleTextBlockUI))
            {
                canvas.Children.Add(macroNode.TitleTextBlockUI);
                // Set initial position
                if (node.Border != null)
                {
                    var titleLeft = node.X + (node.Border.ActualWidth / 2) - (macroNode.TitleTextBlockUI.ActualWidth / 2);
                    var titleTop = node.Y - macroNode.TitleTextBlockUI.ActualHeight - 4;
                    Canvas.SetLeft(macroNode.TitleTextBlockUI, titleLeft);
                    Canvas.SetTop(macroNode.TitleTextBlockUI, titleTop);
                    Panel.SetZIndex(macroNode.TitleTextBlockUI, 20000);
                }
            }
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is MacroRecorderNode macroNode && macroNode.TitleTextBlockUI != null && Host.WorkflowCanvas != null)
            {
                var titleTextBlock = macroNode.TitleTextBlockUI;
        ...
        public void RemoveNode(WorkflowNode node, Canvas canvas)
        {
            ...
            if (node is MacroRecorderNode macroNode && macroNode.TitleTextBlockUI != null)
            {
                var titleTextBlock = macroNode.TitleTextBlockUI;
                ...
                macroNode.TitleTextBlockUI = null;
            }
