...
{
    public sealed class AssignDataNodeRenderer : INodeRenderer
    {
        ...
        public AssignDataNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        {
            if (node is not AssignDataNode assignNode)
                return;

            var window = Host as Window;
            node.Border = AssignDataNodeControl.CreateBorder(assignNode, window, Host);
            ...
            if (assignNode.TitleTextBlockUI != null && !canvas.Children.Contains(assignNode.TitleTextBlockUI))
            {
                canvas.Children.Add(assignNode.TitleTextBlockUI);
                if (node.Border != null)
                {
                    var titleLeft = node.X + (node.Border.ActualWidth / 2) - (assignNode.TitleTextBlockUI.ActualWidth / 2);
                    var titleTop = node.Y - assignNode.TitleTextBlockUI.ActualHeight - 4;
                    Canvas.SetLeft(assignNode.TitleTextBlockUI, titleLeft);
                    Canvas.SetTop(assignNode.TitleTextBlockUI, titleTop);
                    Panel.SetZIndex(assignNode.TitleTextBlockUI, 20000);
                }
            }
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is AssignDataNode assignNode && assignNode.TitleTextBlockUI != null && Host.WorkflowCanvas != null)
            {
                var titleTextBlock = assignNode.TitleTextBlockUI;
        ...
        {
            if (node is AssignDataNode assignNode && assignNode.TitleTextBlockUI != null)
            {
                var titleTextBlock = assignNode.TitleTextBlockUI;
                if (canvas != null && canvas.Children.Contains(titleTextBlock))
                    canvas.Children.Remove(titleTextBlock);
                assignNode.TitleTextBlockUI = null;
            }
        ...
        {
            // Ưu tiên ColorKey của port nếu có (theo guideline Node_Dialog_V2)
