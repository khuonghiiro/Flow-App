...
{
    public sealed class FolderFilePathsNodeRenderer : INodeRenderer
    {
        private readonly PortRenderer _portRenderer;
        private readonly IWorkflowEditorHostAccessor _hostAccessor;
        private IWorkflowEditorHost Host => _hostAccessor.GetRequiredHost();

        public FolderFilePathsNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        {
            if (node is not FolderFilePathsNode dn) return;

            var window = Host as Window;
            node.Border = FolderFilePathsNodeControl.CreateBorder(dn, window, Host);
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is FolderFilePathsNode dn &&
        ...
        public void RemoveNode(WorkflowNode node, Canvas canvas)
        {
            ...
            if (node is FolderFilePathsNode dn && dn.TitleTextBlockUI != null)
            {
                var tb = dn.TitleTextBlockUI;
