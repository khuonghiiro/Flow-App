...
public sealed class FloatingWidgetManager
{
    ...
    private void HideNodeVisualWhenWidgetOpened(WorkflowNode node)
    {
        ...
        if (node is not HtmlUiNode && node is not WebNode && node is not VideoProcessingNode && node is not ImageProcessingNode && node is not ShowInputMsgNode && node is not DynamicUiNode && node is not VideoEditorNode)
            return;
