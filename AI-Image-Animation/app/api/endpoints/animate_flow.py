...
def _compute_flow_sync(task_id: str, req: FlowAnimateRequest):
    """
    ...
    return rel_url, gif_url, file_size


async def _render_flow_task(task_id: str, req: FlowAnimateRequest):
    """
    Asynchronous background handler that offloads CPU work and updates task status on the event loop.
    """
    try:
        rel_url, gif_url, file_size = await anyio.to_thread.run_sync(_compute_flow_sync, task_id, req)
...
async def create_flow_animation(
    ...
    try:
        ...
        background_tasks.add_task(_render_flow_task, task_id, req)
