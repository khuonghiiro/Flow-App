...
namespace FlowMy.Services.Workflow.NodeExecutors
{
    ...
    internal sealed class ScreenCaptureNodeExecutor : INodeExecutor
    {
        ...
        private static async Task ExecuteManualRegionMode(ScreenCaptureNode cap, NodeExecutionEnvironment env)
        {
            ...
            if (cap.UseBackgroundMode && !string.IsNullOrWhiteSpace(cap.TargetProcessName))
            {
                ...
                if (match != null)
                {
                    System.Diagnostics.Debug.WriteLine($"[ExecuteManualRegionMode] Background capture from window: {match.Title}");
                    ...
                    if (GetWindowRect(match.Handle, out RECT rect))
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[ExecuteManualRegionMode] Window size: {windowWidth}x{windowHeight}, Capture region: ({x},{y},{w},{h})");
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ExecuteManualRegionMode] Target window not found: {cap.TargetProcessName}");
                }
            }
            else
            {
        ...
        private static async Task ExecuteScreenCaptureMode(ScreenCaptureNode cap, NodeExecutionEnvironment env)
        {
            ...
            if (cap.UseBackgroundMode && !string.IsNullOrWhiteSpace(cap.TargetProcessName))
            {
                ...
                if (match != null)
                {
                    System.Diagnostics.Debug.WriteLine($"[ExecuteScreenCaptureMode] Background capture from window: {match.Title}");
                    ...
                    if (GetWindowRect(match.Handle, out RECT rect))
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[ExecuteScreenCaptureMode] Window size: {windowWidth}x{windowHeight}, Capture region: ({x},{y},{w},{h})");
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"[ExecuteScreenCaptureMode] Target window not found: {cap.TargetProcessName}");
                }
            }
            else
            {
