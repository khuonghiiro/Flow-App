...
{
    public sealed class ImageProcessingNodeRenderer : INodeRenderer
    {
        private readonly PortRenderer _portRenderer;
        private readonly IWorkflowEditorHostAccessor _hostAccessor;
        private IWorkflowEditorHost Host => _hostAccessor.GetRequiredHost();

        public ImageProcessingNodeRenderer(PortRenderer portRenderer, IWorkflowEditorHostAccessor hostAccessor)
        {
        ...
        {
            if (node is not ImageProcessingNode imageNode) return;

            var window = Host as Window;
            node.Border = ImageProcessingNodeControl.CreateBorder(imageNode, window, Host);
            ...
            if (imageNode.TitleTextBlockUI != null && !canvas.Children.Contains(imageNode.TitleTextBlockUI))
            {
                canvas.Children.Add(imageNode.TitleTextBlockUI);
                Panel.SetZIndex(imageNode.TitleTextBlockUI, 20000);
            }
        ...
        public void UpdateNodePosition(WorkflowNode node, double x, double y)
        {
            ...
            if (node is ImageProcessingNode imageNode && imageNode.TitleTextBlockUI != null && Host.WorkflowCanvas != null)
            {
                var tb = imageNode.TitleTextBlockUI;
        ...
        {
            if (node is ImageProcessingNode imageNode && imageNode.TitleTextBlockUI != null)
            {
                var tb = imageNode.TitleTextBlockUI;
                if (canvas != null && canvas.Children.Contains(tb))
                    canvas.Children.Remove(tb);
                imageNode.TitleTextBlockUI = null;
            }
