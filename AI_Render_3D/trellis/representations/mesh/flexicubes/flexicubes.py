...
def check_tensor(t, shape=None, throw=False):
    if not torch.is_tensor(t):
        if throw: raise TypeError("Not a tensor")
        return False
    if shape is not None:
        if len(shape) != t.dim():
            if throw: raise ValueError(f"Dim mismatch: {len(shape)} vs {t.dim()}")
            return False
        for expected, actual in zip(shape, t.shape):
            if expected is not None and expected != actual:
                if throw: raise ValueError(f"Shape mismatch: {expected} vs {actual}")
                return False
    return True
